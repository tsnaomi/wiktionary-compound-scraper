...
class Extract:
    ...
    def extract(self, orth, href):
        '''Extract details about the word `orth`.

        Details include the word's compound segmentation, part of speech,
        morphosyntactic description (if any), and plurality (if applicable).
        '''
        ...
        if declensions:
            # for compound in self.get_compound(orth, soup):
    ...
    def get_compound(self, orth, soup):
        ...
        for etymology in self.get_etymology(orth, soup):
            ...
            yield '='.join(etymology).replace('-=', '').replace('=-', '')
                ...
                try:
                    if '-' in candidate:
                        for i, comp in enumerate(split):
                            if '-' != comp:
                                p = re.compile(r'-?%s-?' % comp, flags=re.I)
    ...
    def reconcile_etymology(self, split, orth):
        '''Reconcile the etymology (`split`) with `orth`.'''  # TODO
        compound = []
        base = orth
    ...
    def samesies(self, etymology, goal):
        '''Determine if `etymology` and `goal` are equivalent.'''
    ...
    def get_pos(self, soup):
        ...
        headers = [h.text for h in soup.find_all('span', class_='mw-headline')]
        pos = [h for h in headers if h.lower() in POS[self.code]]

        if pos:
            return ' '.join(pos).lower()
    ...
    #         declensions = table.find_all('span', class_='Latn')
    #         declensions = list(set([d.text for d in declensions]))
    ...
    #         elif 'verb' in pos:
    #             n = orth.count(' ')
    #             declensions = list(set(
    #                 d.split(' ', d.count(' ') - n)[-1] for d in declensions))

    #         # attempt to remove the default orthography (which is most likely
    #         # the nominative singular form), so that it can print first in
    #         # self.extract() without duplication
    #         try:
    #             declensions.remove(orth)
